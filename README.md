...
### スプライトのセル対応（移動基準・これが正）

セルは左上=1から右へ、下段へと数えます。**どのセルがどの向きか**は、内部名（facing）だと
「左向き/右向きプロファイル」の見分けで混乱しやすいので、**移動入力（押すキー＝画面方向）で
定義**するのを共通言語とします。下表がコードの実挙動（`getScreenInputVector` →
`INPUT_TO_DIRECTION` → `DIRECTIONS`）と一致する正の対応です。

| セル | キー | 矢印 | 画面入力(x,z) | 内部名(`DIRECTIONS`) | セル位置(col,row) |
|---|---|---|---|---|---|
| 1 | `S` | ↓ | (0, 1) | front | (0,0) |
| 2 | `A+S` | ↙ | (-1, 1) | front_left | (1,0) |
| 3 | `A` | ← | (-1, 0) | left | (2,0) |
| 4 | `A+W` | ↖ | (-1, -1) | back_left | (3,0) |
| 5 | `W` | ↑ | (0, -1) | back | (0,1) |
| 6 | `D+W` | ↗ | (1, -1) | back_right | (1,1) |
| 7 | `D` | → | (1, 0) | right | (2,1) |
| 8 | `D+S` | ↘ | (1, 1) | front_right | (3,1) |

- スプライトの選択は**画面入力**で決まり、実際の移動先だけがカメラ相対（`getCameraRelativeGridDelta`）。
  さらにキャラはビルボードで常にカメラを向くため、**カメラを回しても「`S`＝セル1」は不変**。
- 内部名（`front` 等）は3D手足・ハンマーの向き計算（`FACING`）で意味を持つのでコード上は残す。
  人同士で方向を指すときは「セル7＝`D`／→」のように**セル番号かキー**で言うと曖昧さがない。
